...
function normalizeFuelType(fuel) {
  ...
  if (value.includes("бенз") || value.includes("gasoline") || value.includes("petrol")) return "benz";
  if (value.includes("диз") || value.includes("diesel")) return "diesel";
  if (value.includes("элект") || value.includes("electric") || value.includes("ev")) return "electro";
  return "benz";
}

function isTrailerCategory(category) {
  const value = String(category || "").trim().toLowerCase();
  return value.startsWith("o1") || value.startsWith("o2") || value.startsWith("o3") || value.startsWith("o4");
}
...
function getTemplateKey(protocol) {
  ...
  if (category === "o1" || category === "o2" || category === "o3" || category === "o4") return "o";
  if (category === "n3") {
...
function getTemplatePath(protocol) {
  if (protocol.pdfTemplate) return path.join(__dirname, "..", "templates", protocol.pdfTemplate);
...
function drawMultilineText(page, text, x, y, maxWidth, font, size, lineGap = 1) {
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    if (w > maxWidth && line) {
      page.drawText(line, { x, y: y - lines * (size + lineGap), size, font });
      lines++;
      line = words[i];
    } else {
      line = testLine;
...
function buildM1GeneratedValues() {
  return {
    page4: { r1: randInt(729, 780), r2: randInt(690, 727), r3: oneDec(randInt(700, 850) / 10), r4: randInt(729, 780), r5: randInt(690, 727) },
    page5: { r1: oneDec(randInt(650, 750) / 10), r2: randInt(721, 780), r3: randInt(690, 719), r4: randInt(720, 780), r5: randInt(690, 719), r6: oneDec(randInt(80, 120) * 1000), r7: randInt(65, 100) },
    page6: { r1: randInt(50, 150) / 100, r2: randInt(180, 360), r3: randInt(77, 95), r4: randInt(20, 60) / 10, r5: randInt(20, 60) / 10 },
    page11: { r1: randInt(700, 719), r2: randInt(724, 740) },
  };
}
...
function buildN3GeneratedValues() {
  ...
  return {
    page4: { r1: m1.page4.r1, r2: m1.page4.r2, r3: m1.page4.r3, r4: m1.page4.r4, r5: m1.page4.r5, r6: randInt(721, 780), r7: randInt(690, 719), r8: oneDec(randInt(80, 120) * 1000), r9: randInt(65, 100) },
    page5: { r1: randInt(50, 150) / 100, r2: randInt(25, 90) / 100, r3: randInt(77, 95), r4: randInt(150, 400), r5: randInt(15, 40) / 10 },
    page6: { r1: randInt(80, 99), r2: randInt(0, 90) / 10 },
    page13: { r1: randInt(700, 719), r2: randInt(724, 740) },
  };
}

function buildOTrailerGeneratedValues() {
  return {
    page2: { r1: oneDec(randInt(700, 850) / 10), r2: randInt(500, 600), r3: randInt(620, 680), r4: randInt(12, 20) / 10 },
    page3: { r1: randInt(650, 710), r2: randInt(720, 780) },
    page4: { r1: randInt(40, 80), r2: randInt(110, 150), r3: randInt(100, 300), r4: randInt(40, 90), r5: randInt(200, 420) },
  };
}
...
function getGeneratedValues(protocol) {
  ...
  if (category === "o1" || category === "o2" || category === "o3" || category === "o4" || category === "o") return buildOTrailerGeneratedValues();
  return buildM1GeneratedValues();
}

function drawValue(page, cfg, value, font) {
  if (!cfg) return;
  page.drawText(String(value ?? ""), { x: cfg.x, y: cfg.y, size: cfg.size, font });
}

function drawPage1(page, p1, protocol, font) {
  const rc = p1.rightColumn;
  const protocolNumberText = `${protocol.protocolNumber ?? ""}`;
  page.drawText(protocolNumberText, { x: p1.protocolNumber.x, y: p1.protocolNumber.y, size: p1.protocolNumber.size, font });
  page.drawText(protocolNumberText, { x: p1.protocolNumber.x + (p1.protocolNumber.boldOffsetX || 0), y: p1.protocolNumber.y, size: p1.protocolNumber.size, font });

  const dateText = formatDateRu(protocol.protocolDate);
  page.drawText(dateText, { x: p1.protocolDate.x, y: p1.protocolDate.y, size: p1.protocolDate.size, font });
  page.drawText(dateText, { x: p1.protocolDate.x + (p1.protocolDate.boldOffsetX || 0), y: p1.protocolDate.y, size: p1.protocolDate.size, font });
  ...
  function putRight(value, opts = {}) {
    ...
    page.drawText(text, { x: xRight, y: yRight, size: opts.size ?? rc.defaultSize ?? 6, font });
    yRight -= step;
  }
  ...
  function putRightWrap(value, opts = {}) {
    ...
    const usedLines = drawMultilineText(page, text, xRight, yRight, maxWidth, font, size, gap);
    yRight -= usedLines * (size + gap) + 2;
  }
  ...
  if (!isTrailerCategory(protocol.category)) putRight(protocol.EcologicalClass, { size: rc.defaultSize ?? 6 });
  putRight(`${protocol.year ?? ""} г.в.`, { size: rc.yearSize ?? rc.defaultSize ?? 6 });
  putRight(formatDateRu(protocol.protocolDate), { size: rc.defaultSize ?? 6 });
  yRight -= step * (rc.afterFirstDateGap ?? 0.01);
  putRight(formatDateRu(protocol.protocolDate), { size: rc.defaultSize ?? 6 });
  yRight -= step * (rc.afterSecondDateGap ?? 2.6);
  putRight(protocol.fio, { size: rc.fioSize ?? rc.defaultSize ?? 6 });
  yRight -= step * (rc.afterFioGap ?? 0.2);
  putRightWrap(protocol.address, { size: rc.addressSize ?? 6, maxWidth: rc.addressMaxWidth ?? 165, gap: 1 });
  yRight -= step * (rc.afterAddressGap ?? 0.8);

  page.drawText(String(protocol.MANUFACTURER || ""), { x: p1.manufacturer.x, y: p1.manufacturer.y, size: p1.manufacturer.size, font });
  drawMultilineText(page, protocol.legaladdressoftheMANUFACTURER || "", p1.manufacturerAddress.x, p1.manufacturerAddress.y, p1.manufacturerAddress.maxWidth, font, p1.manufacturerAddress.size, p1.manufacturerAddress.lineGap);
  page.drawText(String(protocol.ASSEMBLYPLANT || ""), { x: p1.assemblyPlant.x, y: p1.assemblyPlant.y, size: p1.assemblyPlant.size, font });
  drawMultilineText(page, protocol.addressoftheassemblyplant || "", p1.assemblyPlantAddress.x, p1.assemblyPlantAddress.y, p1.assemblyPlantAddress.maxWidth, font, p1.assemblyPlantAddress.size, p1.assemblyPlantAddress.lineGap);

  page.drawText(`${protocol.temperature || ""} °C`, { x: p1.conditions.x, y: p1.conditions.temperatureY, size: p1.conditions.size, font });
  page.drawText(`${protocol.humidity || ""} %`, { x: p1.conditions.x, y: p1.conditions.humidityY, size: p1.conditions.size, font });
  page.drawText(`${protocol.pressure || ""} мм рт. ст.`, { x: p1.conditions.x, y: p1.conditions.pressureY, size: p1.conditions.size, font });
}

function dimensions(protocol) {
  return {
    w: protocol.width ?? protocol.Width ?? "",
    h: protocol.height ?? protocol.Height ?? "",
    l: protocol.length ?? protocol.Length ?? "",
  };
}

export async function generateTemplatePdfBuffer(protocol) {
  const templatePath = getTemplatePath(protocol);
  if (!fs.existsSync(templatePath)) throw new Error(`Шаблон не найден: ${templatePath}`);
  ...
  const fuelKey = normalizeFuelType(protocol.fuelType || protocol.fuel);
  const categoryKey = normalizeCategory(protocol.templateCategory || protocol.category);
  const templateKey = getTemplateKey(protocol);
  const layout = protocolPdfLayouts[templateKey];
  const generated = getGeneratedValues(protocol);
  if (!layout) throw new Error(`Нет layout для шаблона: ${templateKey}`);

  if (layout.page1 && pages[0]) drawPage1(pages[0], layout.page1, protocol, font);
  ...
  } else if (layout.page2?.extraEquipment && pages[1]) {
    const page2 = pages[1];
    const p2 = layout.page2;
    drawMultilineText(page2, protocol.extraEquipment || "", p2.extraEquipment.x, p2.extraEquipment.y, p2.extraEquipment.maxWidth, font, p2.extraEquipment.size);
  }
  ...
  if (categoryKey === "n3" && layout.page5 && pages[4]) {
    const page5 = pages[4];
  ...
  } else if ((categoryKey === "m1" || categoryKey === "n1" || categoryKey === "n2") && layout.page5 && pages[4]) {
    const page5 = pages[4];
    const p5 = layout.page5;
    drawValue(page5, p5.r1, generated.page5.r1, font);
    drawValue(page5, p5.r2, generated.page5.r2, font);
  ...
  if (categoryKey === "n3" && layout.page6 && pages[5]) {
    const page6 = pages[5];
    const p6 = layout.page6;
    drawValue(page6, p6.r1, generated.page6.r1, font);
    drawValue(page6, p6.r2, generated.page6.r2, font);
  } else if ((categoryKey === "m1" || categoryKey === "n1" || categoryKey === "n2") && layout.page6 && pages[5]) {
    const page6 = pages[5];
    const p6 = layout.page6;
    drawValue(page6, p6.r1, generated.page6.r1, font);
    drawValue(page6, p6.r2, generated.page6.r2, font);
    ...
    if (categoryKey === "n2" && p6.r6) drawValue(page6, p6.r6, randInt(25, 90) / 100, font);
  }

  if ((categoryKey === "m1" || categoryKey === "n1") && layout.page9 && pages[8]) {
    const page9 = pages[8];
    ...
    if (fuelKey === "diesel") drawValue(page9, p9.smoke, protocol.smokeValue || "", font);
  }

  if (categoryKey === "n2" && layout.page9 && pages[8]) {
    const page9 = pages[8];
    const p9 = layout.page9;
    if (p9.year) drawValue(page9, p9.year, protocol.year || "", font);
    if (p9.yearSuffix) drawValue(page9, p9.yearSuffix, "г.в.", font);
  }

  if ((categoryKey === "m1" || categoryKey === "n1") && layout.page10 && pages[9]) {
    const page10 = pages[9];
    const p10 = layout.page10;
    const { w, h, l } = dimensions(protocol);
  ...
  if (categoryKey === "n2" && layout.page10 && pages[9]) {
    const page10 = pages[9];
    const p10 = layout.page10;
    if (fuelKey === "diesel" && p10.smoke) drawValue(page10, p10.smoke, protocol.smokeValue || "", font);
    if (fuelKey === "benz") {
  ...
  if ((categoryKey === "m1" || categoryKey === "n1") && layout.page11 && pages[10]) {
    const page11 = pages[10];
    const p11 = layout.page11;
    if (fuelKey !== "electro" && p11.noise) drawValue(page11, p11.noise, protocol.noiseValue || "", font);
    if (categoryKey === "m1") {
  ...
  if (categoryKey === "n2" && layout.page12 && pages[11]) {
    const page12 = pages[11];
    const p12 = layout.page12;
    if (fuelKey === "diesel" && p12.smoke) drawValue(page12, p12.smoke, protocol.smokeValue || "", font);
    if (p12.noise) drawValue(page12, p12.noise, protocol.noiseValue || "", font);
  }
...
export async function generateProtocolPdfBuffer(protocol) {
  ...
  const getVal = (label) => {
    const arr = Array.isArray(protocol.identificationResults) ? protocol.identificationResults : [];
  ...
  autoTable(doc, {
    ...
    ...grid,
    body: [
      [{ content: "", styles: { minCellHeight: 22 } }, { content: protocol.countryText || "", styles: { halign: "left", minCellHeight: 22 } }, { content: protocol.accreditationText || "", styles: { halign: "right", minCellHeight: 22 } }],
      [{ content: LAB_ADDRESS, colSpan: 3, styles: { halign: "center", minCellHeight: 10, valign: "middle" } }],
    ],
    columnStyles: { 0: { cellWidth: usableWidth * 0.16 }, 1: { cellWidth: usableWidth * 0.42 }, 2: { cellWidth: usableWidth * 0.42 } },
    styles: { ...grid.styles, cellPadding: 1.0 },
    didDrawCell: (data) => {
      if (!logoBase64 || data.row.index !== 0 || data.column.index !== 0) return;
      ...
      doc.addImage(logoBase64, "PNG", cell.x + (cell.width - w) / 2, cell.y + (cell.height - h) / 2, w, h);
    },
  });
  ...
  doc.setFont("Times", "normal");
  doc.setFontSize(11);
  doc.text(`технической экспертизы и испытаний единичного транспортного средства № ${numStr} от ${dateStr}`, pageWidth / 2, y, { align: "center", maxWidth: usableWidth });
  y += 7;
  doc.text("На соответствие требованиям ТР ТС 018/2011 «О безопасности колесных транспортных средств» п. 11-14, приложение № 4-6, п. 4 приложение 7", pageWidth / 2, y, { align: "center", maxWidth: usableWidth });
  y += 12;
  ...
  const idRows = normalizedCategory === "o" ? idRowsBase.filter(([label]) => label !== "Экологический класс") : idRowsBase;
  ...
  autoTable(doc, {
    ...
    body: idRows.map(([l, v]) => [{ content: l || "", styles: { halign: "center" } }, { content: v || "", styles: { halign: "center" } }]),
    columnStyles: { 0: { cellWidth: usableWidth * 0.7 }, 1: { cellWidth: usableWidth * 0.3 } },
  });
  ...
  autoTable(doc, {
    ...
    margin: { left: margin, right: margin },
    ...grid,
    head: [[{ content: "УСЛОВИЯ ПРОВЕДЕНИЯ ИСПЫТАНИЙ", styles: { halign: "left" } }, { content: "Значение", styles: { halign: "left" } }]],
    body: condRows,
    columnStyles: { 0: { cellWidth: usableWidth * 0.7 }, 1: { cellWidth: usableWidth * 0.3 } },
  });
  ...
  autoTable(doc, {
    ...
    body: equipmentRows,
    columnStyles: { 0: { cellWidth: usableWidth * 0.15 }, 1: { cellWidth: usableWidth * 0.5 }, 2: { cellWidth: usableWidth * 0.35 } },
  ...
  if (normalizedCategory === "m1") {
    if (fuel.includes("бенз")) {
      if (protocol.noiseValue != null) extra.push({ parameter: "Уровень шума", method: "-", normValue: "-", actualValue: String(protocol.noiseValue) });
      if (protocol.gasValue != null) extra.push({ parameter: "Выбросы газа", method: "-", normValue: "-", actualValue: String(protocol.gasValue) });
    } else if (fuel.includes("диз")) {
      if (protocol.noiseValue != null) extra.push({ parameter: "Уровень шума", method: "-", normValue: "-", actualValue: String(protocol.noiseValue) });
      if (protocol.smokeValue != null) extra.push({ parameter: "Выбросы дыма", method: "-", normValue: "-", actualValue: String(protocol.smokeValue) });
    }
  }
  ...
  const resultsRows = [...filtered, ...extra].map((r) => [r.parameter || "", r.method || "", r.normValue || "", r.actualValue || ""]);
  ...
  autoTable(doc, {
    ...
    body: resultsRows,
    columnStyles: { 0: { cellWidth: usableWidth * 0.15 }, 1: { cellWidth: usableWidth * 0.15 }, 2: { cellWidth: usableWidth * 0.4 }, 3: { cellWidth: usableWidth * 0.3 } },
    styles: { ...grid.styles, fontSize: 9, cellPadding: 2.0, valign: "middle" },
  });
